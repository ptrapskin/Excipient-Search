...
function start(e) { drawing = true; hasSig = true; const p = pos(e); ctx.beginPath(); ctx.moveTo(p.x, p.y); document.getElementById('sigStatus').textContent = 'Signed'; document.getElementById('sigStatus').style.color = '#1a7a3a'; document.querySelector('.cl-sig-pad-wrap')?.classList.remove('field-missing'); e.preventDefault(); }
...
const REQUIRED_HEADER_FIELDS = [
  ['sourceProgram', 'a source program'],
...
document.getElementById('generateBtn').addEventListener('click', () => {
  for (const [id, label] of REQUIRED_HEADER_FIELDS) {
    const el = document.getElementById(id);
    if (!el.value) { warnMissingField(el, `Select/enter ${label} before generating the record`); return; }
  }
  ...
  if (!hasSig) {
    warnMissingField(document.querySelector('.cl-sig-pad-wrap'), 'Signature is required before generating the record');
    return;
  }
